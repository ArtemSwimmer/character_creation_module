...
def special(char_name, char_class):
    special = ('применил специальное умение')
    if char_class == 'warrior':
        return (f'{char_name} {special} «Выносливость {80 + 25}»')
    if char_class == 'mage':
        return (f'{char_name} {special} «Атака {5 + 40}»')
    if char_class == 'healer':
        return (f'{char_name} {special} «Защита {10 + 30}»')
    return None
